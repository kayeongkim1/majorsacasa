...
public class SocialWorkerDAO {
	...
	public SocialWorker getSocialWorker(String UserCAS){
		try{
			return jdbcTemplate.queryForObject("SELECT * FROM SOCIALWORKER WHERE DNI = ?", new SocialWorkerRowMapper(), UserCAS);
	...
	public List<SocialWorker> getSocialWorkers(){
		try{
			return jdbcTemplate.query("SELECT * FROM SOCIALWORKERS", new SocialWorkerRowMapper());
		}catch (EmptyResultDataAccessException e){
